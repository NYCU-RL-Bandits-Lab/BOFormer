...
class QT(torch.nn.Module):
    def __init__(self, 
        ...
        # self.predict_q_value = nn.Sequential(*[torch.nn.Linear(hidden_size, 1), torch.nn.Softplus()])
        ...
        for pn, p in self.named_parameters():
            if pn.endswith('c_proj.weight'):
                #torch.nn.init.trunc_normal_(p, mean=1.0/(1.0-self.gamma), std=0.02, a=0, b=1.0/(1.0-self.gamma)+0.02)
                torch.nn.init.normal_(p, mean=10, std=0.02/math.sqrt(2 * n_layer))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--training_episode', type=int, default=5002)
    ...
    parser.add_argument('--bc_rate', type=float, default="0.0", help='rate of BC')
    parser.add_argument('--record_idx', type=int, default="-1", help='rate of BC')
    args = parser.parse_args()
    if args.record_idx == -1:
        record_index = 0
        while pathlib.Path(f"BOFormer_models/model{record_index}").exists():
            record_index += 1
        writer = SummaryWriter(f"logs/logs{record_index}") # tensorboard --logdir=logs{i}
        if not os.path.exists(f"records/"):
            os.mkdir(f"records")
        if not os.path.exists(f"BOFormer_models/"):
            os.mkdir(f"BOFormer_models")
        print(f"record index: {record_index}")
        os.mkdir(f"records/record{record_index}")
        os.mkdir(f"BOFormer_models/model{record_index}")
        f = open("BOFormer_record.txt", "a")
        f.write(str(record_index) + "\t" + str(args) + "\n")
        f.close()
    else:
        record_index = args.record_idx
        writer = SummaryWriter(f"logs/logs{record_index}")
    MOBO_info = "{}_domain_{}_T_{}_fnum_{}_gamma_{}_hidden_{}_lr_{}_weight_decay_{}_seed_{}_n_layer_{}_n_head_{}_n_positions_{}_dropout_{}_epsilon_{}_target_update_freq_{}_batch_size_{}_demo_rate_{}_buffer_size_{}_new_reward_{}_sample_rate_{}".format(
        args.function_type,
        args.domain_size, 
        args.T, 
        args.f_num, 
    ...
    learner = QT(T = args.T, 
                 ...
                 bc_rate = args.bc_rate,
                 device = device).to(device)
     
    if args.model_episode > 0:
        learner.load_state_dict(torch.load("./BOFormer_models/model{}/{}.pth".format(record_index, args.model_episode)))
    learner.train()
    ...
    for e in range(args.training_episode):
        ...
        if demo:
            ...
            env.history["q_values"] = learner.previous_q_values
        writer.add_scalar("train/final_regret", regret, e)
        writer.add_scalar("train/hv", env.history['hypervolume_true'][-1], e)
        ...
        if demo or e < args.demo_time:
            print('EP:{} | Loss: {:.3f} | P: {} | R: {:.3f}'.format(e + args.model_episode, np.mean(losses), args.demo_policy, regret))
        else:
            print('EP:{} | Loss: {:.3f} | Q: {:.3f} | R: {:.3f}'.format(e + args.model_episode, np.mean(losses), torch.min(learner.previous_q_values), regret))

        # save model
        if e % 5 == 0: torch.save(learner.state_dict(), "./BOFormer_models/model{}/{}.pth".format(record_index, e + args.model_episode))

        # save regrets
        filename = '{}.pkl'.format(e + args.model_episode)
        with open(os.path.join(f"records/record{record_index}", filename), 'wb') as f:
            pickle.dump(env.history, f)
        
    torch.save(learner.state_dict(), "./BOFormer_models/model{}/{}.pth".format(record_index, e + args.model_episode))